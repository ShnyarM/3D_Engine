...
public:
	...
	constexpr static _Mat Identity()
	{
		if constexpr (S == 2)
		{
			return { (T)1.0, (T)0.0, (T)0.0, (T)1.0 };
		}
		...
		{
			static_assert(false, "Bad dimension for Identity Matrix");
		}
	}
	static _Mat Scaling(T factor)
	{
		if constexpr (S == 2)
		{
			return { factor, (T)0.0, (T)0.0, factor };
		}
		else if constexpr (S == 3)
		{
			return {
		...
		{
			static_assert(false, "Bad dimension for Scaling Matrix");
		}
	}
	static _Mat RotationX(T theta)
	{
		...
		{
			static_assert(false, "Bad dimension for RotationX Matrix.");
		}
	}
	static _Mat RotationY(T theta)
	{
		...
		{
			static_assert(false, "Bad dimension for RotationY Matrix.");
		}
	}
	static _Mat RotationZ(T theta)
	{
		...
		{
			static_assert(false, "Bad dimension for RotationZ Matrix.");
		}
	}
	template<class V>
	static _Mat Translation(const V& tv)
	{
...
typedef _Mat<float, 2> Mat2;
typedef _Mat<double, 2> Mad2;
